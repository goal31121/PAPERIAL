...
class TestIndex(unittest.TestCase):
    ...
    def testStream(self):
        ...
        # Full index stream
        self.assertEqual(len(list(Index.stream(Utils.db_file, 0, 0, True))), 29218)

        # Partial index stream - top n documents by entry date
        self.assertEqual(len(list(Index.stream(Utils.db_file, 10, 0, True))), 287)

        # Partial index stream - top n documents by citation count
        self.assertEqual(len(list(Index.stream(Utils.db_file, 0, 10, True))), 108)
